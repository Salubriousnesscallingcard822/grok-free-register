...
''', encoding="utf-8")
print("PATH A: wrote", browser)

# ---------- PATH B: closed loop device flow + browser approve + poll ----------
loop_script = root / "scripts" / "device_flow_browser_complete.py"
loop_script.write_text(r'''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
if __name__ == "__main__":
    raise SystemExit(main())
''', encoding="utf-8")
print("PATH B: wrote", loop_script)

# Fix coordinator oauth_rejected prefix mapping
coord = root / "xai_enroller" / "coordinator.py"
ct = coord.read_text(encoding="utf-8")
old = '''            mapping = {
            ...
            if status is None:
                for key, value in mapping.items():
                    if reason.startswith(key):
                        ...
                        break
            result = JobResult(
                source.source_id, status or JobStatus.TRANSPORT_FAILED, reason, attempt
            )
'''
if old in ct:
    coord.write_text(ct.replace(old, new), encoding="utf-8")
    print("patched coordinator mapping")
else:
    print("coordinator mapping pattern not found")

# docs update
doc = root / "docs" / "guides" / "browser-device-authorize.md"
doc.write_text('''# accounts.x.ai 浏览器批准（双路）
...
# make scripts package importable lightly
