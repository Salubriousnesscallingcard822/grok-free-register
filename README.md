...
| 模式 | 作用 |
|------|------|
| `all`（默认） | 注册 + Path B 浏览器 OAuth + 导入 grok2api |
| `register-only` | 仅 tempmail 注册 |
| `auth-only` | 仅 accounts.x.ai 浏览器批准 → `auth-local/authenticated` |
| `import-only` | 仅把 `xai-*.json` 导入本地 grok2api |
| `status` / `stop` | 状态 / 停止本链路进程 |

管理端账号写在 `keys/.credentials`（`ADMIN_USER` / `ADMIN_PASS`）。  
详见 `PIPELINE.md` 与 `scripts/import_authenticated_to_grok2api.py`。


## Windows modern stack (CloakBrowser + one-click import)

Recent features are now first-class in this repo:

| Feature | Entry |
|---|---|
| CloakBrowser anti-detect browser | start-windows.ps1 (CLOAKBROWSER_CACHE_DIR=.cloakbrowser) |
| Path B OAuth (ccounts.x.ai) | start-auth-windows.ps1 |
| One-click import to grok2api | scripts/import_authenticated_to_grok2api.py |
| Full chain register->auth->import | start-full-to-grok2api.ps1 |
| Unified entry | start-all-windows.cmd / start_all_windows.py |
| Custom mailbox long-run | start-email-service-windows.ps1 + CF Worker |
...
Docs:
- [CloakBrowser + import](docs/guides/cloakbrowser-and-import.md)
- [Full chain](docs/guides/full-to-grok2api.md)
- [Custom mailbox](docs/guides/custom-mailbox-yanqiudesu.md)
