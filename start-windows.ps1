...
if (-not (Test-Path ".env")) {
  ...
  Set-Content -Path ".env" -Value "EMAIL_MODE=tempmail`nTEMPMAIL_PROVIDER_ORDER=lol,mailtm`nHTTP_PROXY=$($env:HTTP_PROXY)`nHTTPS_PROXY=$($env:HTTPS_PROXY)`nTARGET=0`nREGISTER_LOG_MODE=user`nPHYSICAL_CAP=6`nT_SLOT_CAP=16`nQ_SLOT_CAP=16`nQ_PENDING_CAP=24`nP_BATCH_MAX=6" -Encoding ASCII
}
