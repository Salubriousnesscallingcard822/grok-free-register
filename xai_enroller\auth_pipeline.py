...
class AuthPipeline:
    ...
    async def _complete(self, completion):
        ...
        try:
            ...
            reason = str(error)
            _LOG.warning(
                "token completion RuntimeError job=%s attempt=%s reason=%s",
                prepared.job_id,
                prepared.attempt_number,
                reason,
                exc_info=True,
            ...
            self._completion_cancellable = False
            _LOG.warning(
                "token completion transport error job=%s attempt=%s error=%s",
                prepared.job_id,
                prepared.attempt_number,
                error,
                exc_info=True,
            ...
            self._completion_cancellable = False
            _LOG.exception(
                "token completion unexpected error job=%s attempt=%s error=%s",
                prepared.job_id,
                prepared.attempt_number,
                error,
            )
        ...
        try:
            ...
            _LOG.exception(
                "sink store failed job=%s attempt=%s error=%s",
                prepared.job_id,
                prepared.attempt_number,
                error,
            )
