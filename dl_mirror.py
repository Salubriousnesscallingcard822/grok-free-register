...
html = opener.open(f"{MIRROR}/{pkg}/", timeout=20).read().decode("utf-8", "ignore")
matched = []
for href in re.findall(r'href="([^"]+)"', html):
    ...
    if re.fullmatch(pattern, name) and ".dev" not in name:
        if clean.startswith("http"):
            ...
            matched.append(urllib.request.urljoin(f"{MIRROR}/{pkg}/", clean))
if not matched:
...
with opener.open(req, timeout=30) as resp:
    ...
    print(f"{name} +{got} total={size} full={total} cl={cl}")
