    ...
    async def poll_token(self, *, endpoint, flow, timeout):
        ...
        while time.monotonic() < deadline:
            try:
                ...
                # Windows proxy stacks occasionally drop keep-alive sockets mid-poll.
                # Retry inside the same OAuth window instead of failing the whole job.
                transport_failures += 1
                if transport_failures >= 8 or time.monotonic() + interval >= deadline:
                    raise
                await self.sleep(min(max(interval, 1.0), 5.0) + min(transport_failures, 3))
                continue
            document = await self._json(response)
            transport_failures = 0
            if response.status_code // 100 == 2:
