...
class PlaywrightExecutor:
    ...
    def _find_executable_path():
        ...
        if configured and os.path.exists(configured):
            return configured
        roots = []
        cache_dir = (os.environ.get("CLOAKBROWSER_CACHE_DIR") or "").strip()
        if cache_dir:
            roots.append(cache_dir)
        roots.append(os.path.expanduser("~/.cloakbrowser"))
        # project-local cloakbrowser next to package root
        roots.append(str(Path(__file__).resolve().parents[1] / ".cloakbrowser"))
        candidates = []
        patterns = [
            "chromium-*/chrome",
            "chromium-*/chrome.exe",
