...
def build_http_client(timeout=60.0):
    """Build AsyncClient with explicit proxy support for Windows proxy stacks."""
    import httpx
    import os
    ...
    timeout_cfg = httpx.Timeout(
        ...
        read=float(timeout),
        write=min(20.0, float(timeout)),
        pool=min(20.0, float(timeout)),
    )
...
class InteractiveCommandPrompt:
    ...
    def attach(self, loop, callback):
        ...
        # Windows asyncio has no add_reader for stdin pipes/console.
        # Fall back to a daemon thread that pushes lines into the loop.
        use_thread = os.name == "nt"
        if not use_thread:
            try:
                loop.add_reader(self._fd, lambda: None)
            ...
            def reader():
                try:
                    while True:
                        ...
                        if line == "":
                            # EOF on redirected/non-interactive stdin must NOT stop the service.
                            # Only explicit "q" should shut it down.
                            if self.interactive and self._callback is not None:
                                loop.call_soon_threadsafe(self._callback, None)
                            break
                        if self._callback is not None:
                            loop.call_soon_threadsafe(self._callback, line)
                except Exception:
                    # ignore broken pipe / closed stdin in daemon mode
                    if self.interactive and self._callback is not None:
                        with suppress(Exception):
                            loop.call_soon_threadsafe(self._callback, None)

            import threading
            self._reader_thread = threading.Thread(target=reader, name="auth-stdin", daemon=True)
...
def prepare_local_service_environment(env=None):
    ...
    # POSIX mode bits are best-effort; Windows often rejects chmod/fchmod.
    try:
...
async def _run_interactive(runner, terminal):
    ...
    prompt.write_event(
        "命令：s 状态 | take N 取用凭证 | p 暂停 | r 恢复 | c 取消当前任务 | q 退出"
    )
    try:
    ...
    try:
        while not worker.done():
            ...
            if command is None:
                # non-interactive EOF: keep running as daemon
...
async def _run_daemon(runner, terminal):
    """Headless mode for Windows background launch (no TTY / redirected stdin)."""
    terminal.emit(("control", {"state": "daemon"}))
    worker = asyncio.create_task(runner.run())
    try:
        await worker
    except asyncio.CancelledError:
...
async def main_async(*, log_mode="user"):
    ...
    try:
        ...
        daemon = (
            (os.environ.get("XAI_AUTH_SERVICE_DAEMON") or "").strip().lower() in {"1", "true", "yes"}
            or not sys.stdin.isatty()
        )
        if daemon:
            await _run_daemon(runner, terminal)
        else:
