...
用法:
  bash start.sh          # 一键引导
"""
import os, json, random, string, time, re, secrets, base64, struct, asyncio, glob, sys, multiprocessing, threading
...
class _ProxySession(req.Session):
    def request(self, method, url, **kwargs):
        kwargs.setdefault("proxies", _requests_proxies())
...
def _browser_proxy_settings():
    """Playwright/CloakBrowser must set proxy explicitly; env alone is not enough on Windows."""
    proxy = (
        (os.environ.get("HTTPS_PROXY") or "").strip()
        or (os.environ.get("https_proxy") or "").strip()
        or (os.environ.get("HTTP_PROXY") or "").strip()
...
def find_chrome():
    cache_roots = []
    custom = (os.environ.get("CLOAKBROWSER_CACHE_DIR") or "").strip()
    if custom:
        cache_roots.append(custom)
    cache_roots.append(os.path.expanduser("~/.cloakbrowser"))
    cache_roots.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".cloakbrowser"))
    patterns = []
...
def get_system_resources(max_mem_arg=None):
    ...
    # Linux
    try:
        out = subprocess.check_output(["free", "-m"], stderr=subprocess.DEVNULL).decode()
        for line in out.split("\n"):
            if line.lower().startswith("mem"):
                parts = line.split()
                total, available = int(parts[1]), int(parts[6] if len(parts) > 6 else parts[3])
                break
    except Exception:
        # Windows / fallback
        if platform.system().lower().startswith("win"):
            try:
                ...
                if ctypes.windll.kernel32.GlobalMemoryStatusEx(ctypes.byref(stat)):
                    total = max(1, int(stat.ullTotalPhys // (1024 * 1024)))
                    available = max(1, int(stat.ullAvailPhys // (1024 * 1024)))
            except Exception:
                try:
                    ...
                    data = json.loads(out)
                    total = max(1, int(data.get("TotalVisibleMemorySize", 0) // 1024))
                    available = max(1, int(data.get("FreePhysicalMemory", 0) // 1024))
                except Exception:
                    total, available = 4096, 2048
        else:
            total, available = 4096, 2048
    ...
    if max_mem_arg:
        ...
        # Keep a little headroom for OS/UI on Windows desktops.
        max_mem = available
...
async def fetch_config():
    ...
    async with async_playwright() as pw:
        browser = await pw.chromium.launch(executable_path=find_chrome(), headless=True, proxy=_browser_proxy_settings())
        try:
...
async def grpc_create_code(page, email):
    ...
    s = await page.evaluate(f"(async()=>{{var fb=Uint8Array.from(atob('{fb64}'),c=>c.charCodeAt(0));var r=await fetch('{SITE_URL}/auth_mgmt.AuthManagement/CreateEmailValidationCode',{{method:'POST',headers:{{'content-type':'application/grpc-web+proto','x-grpc-web':'1','x-user-agent':'connect-es/2.1.1'}},body:fb.buffer}});var st=r.headers.get('grpc-status')||'0';var msg=r.headers.get('grpc-message')||'';return st+'|'+msg;}})()")
    status, _, msg = (s or '1|').partition('|')
    if status != '0':
        debug_log(f"[P] create_code rejected email={email} status={status} msg={msg[:180]}")
        msg_l = (msg or '').lower()
        if 'domain' in msg_l and 'reject' in msg_l:
            _mark_domain_rejected(email, reason=msg[:120])
    return status == '0'
...
# =============================================================================
# 免费临时邮箱 provider:
#  - tempmailio: api.internal.temp-mail.io (当前可建箱/收信)
#  - mail.tm 同协议: mail.tm / mail.gw / duckmail.sbs
#  - tempmail.lol
# handle 编码 provider, 供 poll_code 分派。
TEMPMAIL_BASES = [
    x.strip()
    for x in (
        os.environ.get("TEMPMAIL_BASES")
        or "https://api.mail.tm,https://api.mail.gw,https://api.duckmail.sbs"
    ).split(",")
    if x.strip()
]
# x.ai 会拒绝大量公共临时邮箱域名。默认优先 tempmailio + mail.tm, 再回落 lol。
TEMPMAIL_PROVIDER_ORDER = (
    os.environ.get("TEMPMAIL_PROVIDER_ORDER") or "lol,io,mailtm"
).strip().lower()
TEMPMAIL_DOMAIN_BLACKLIST = {
    x.strip().lower()
    for x in (
        os.environ.get("TEMPMAIL_DOMAIN_BLACKLIST") or "duckmail.sbs"
    ).split(",")
    if x.strip()
}
EMAIL_CREATE_MIN_INTERVAL = max(
    0.0, float(os.environ.get("EMAIL_CREATE_MIN_INTERVAL") or "0.35")
)
EMAIL_PROVIDER_COOLDOWN_SEC = max(
    5, _env_int("EMAIL_PROVIDER_COOLDOWN_SEC", 45)
)
EMAIL_CREATE_MAX_ATTEMPTS = max(1, _env_int("EMAIL_CREATE_MAX_ATTEMPTS", 6))
CREATE_CODE_MAX_RETRIES = max(1, _env_int("CREATE_CODE_MAX_RETRIES", 3))

_REJECTED_EMAIL_DOMAINS = set(TEMPMAIL_DOMAIN_BLACKLIST)
_REJECTED_EMAIL_DOMAINS_LOCK = threading.Lock()
_DOMAIN_BLACKLIST_FILE = Path(
    os.environ.get("TEMPMAIL_DOMAIN_BLACKLIST_FILE")
    or "keys/rejected-email-domains.txt"
)
try:
    if _DOMAIN_BLACKLIST_FILE.exists():
        for _line in _DOMAIN_BLACKLIST_FILE.read_text(encoding="utf-8", errors="ignore").splitlines():
            _dom = _line.strip().lower()
            if _dom and not _dom.startswith("#"):
                _REJECTED_EMAIL_DOMAINS.add(_dom)
except Exception:
    pass

_PROVIDER_COOLDOWN_UNTIL = {}
_PROVIDER_COOLDOWN_LOCK = threading.Lock()
_EMAIL_CREATE_LOCK = threading.Lock()
_LAST_EMAIL_CREATE_TS = 0.0


def _email_domain(email_or_domain: str) -> str:
    text_value = (email_or_domain or "").strip().lower()
    if "@" in text_value:
        return text_value.rsplit("@", 1)[-1]
    return text_value


def _domain_is_rejected(email_or_domain: str) -> bool:
    domain = _email_domain(email_or_domain)
    if not domain:
        return True
    with _REJECTED_EMAIL_DOMAINS_LOCK:
        return domain in _REJECTED_EMAIL_DOMAINS


def _mark_domain_rejected(email_or_domain: str, reason: str = "") -> None:
    domain = _email_domain(email_or_domain)
    if not domain:
        return
    with _REJECTED_EMAIL_DOMAINS_LOCK:
        is_new = domain not in _REJECTED_EMAIL_DOMAINS
        _REJECTED_EMAIL_DOMAINS.add(domain)
        if is_new:
            try:
                _DOMAIN_BLACKLIST_FILE.parent.mkdir(parents=True, exist_ok=True)
                with _DOMAIN_BLACKLIST_FILE.open("a", encoding="utf-8") as fh:
                    fh.write(domain + "\n")
            except Exception:
                pass
    if is_new:
        suffix = f" reason={reason}" if reason else ""
        debug_log(f"[P] blacklist email domain={domain}{suffix}")


def _provider_available(name: str) -> bool:
    with _PROVIDER_COOLDOWN_LOCK:
        return time.time() >= float(_PROVIDER_COOLDOWN_UNTIL.get(name, 0.0))


def _provider_cooldown(name: str, seconds: float | None = None, reason: str = "") -> None:
    wait = float(seconds if seconds is not None else EMAIL_PROVIDER_COOLDOWN_SEC)
    until = time.time() + max(1.0, wait)
    with _PROVIDER_COOLDOWN_LOCK:
        prev = float(_PROVIDER_COOLDOWN_UNTIL.get(name, 0.0))
        _PROVIDER_COOLDOWN_UNTIL[name] = max(prev, until)
    suffix = f" reason={reason}" if reason else ""
    debug_log(f"[P] provider cooldown name={name} wait={wait:.1f}s{suffix}")


def _throttle_email_create() -> None:
    global _LAST_EMAIL_CREATE_TS
    if EMAIL_CREATE_MIN_INTERVAL <= 0:
        return
    with _EMAIL_CREATE_LOCK:
        now = time.time()
        wait = EMAIL_CREATE_MIN_INTERVAL - (now - _LAST_EMAIL_CREATE_TS)
        if wait > 0:
            time.sleep(wait)
        _LAST_EMAIL_CREATE_TS = time.time()


def _is_rate_limited_status(status_code: int, body: str = "") -> bool:
    if status_code == 429:
        return True
    text_value = (body or "").lower()
    return any(
        key in text_value
        for key in ("rate limit", "rate_limited", "too many requests", "try again later")
    )
...
def _mailtm_create(base, password):
    ...
    _throttle_email_create()
    d = req.get(f'{base}/domains', timeout=12).json()
    d = d.get('hydra:member', d) if isinstance(d, dict) else d
    if not isinstance(d, list):
        raise RuntimeError(f'bad domains payload from {base}')
    doms = []
    for x in d:
        if not isinstance(x, dict):
            continue
        domain = str(x.get('domain') or '').strip().lower()
        if not domain:
            continue
        if not x.get('isActive', True) or x.get('isPrivate', False):
            continue
        if _domain_is_rejected(domain):
            continue
        doms.append(domain)
    if not doms:
        raise RuntimeError(f'no usable domain from {base}')
    last_err = None
    for domain in doms:
        email = f'oc{secrets.token_hex(5)}@{domain}'
        cr = req.post(f'{base}/accounts', json={'address': email, 'password': password}, timeout=12)
        if _is_rate_limited_status(cr.status_code, cr.text):
            _provider_cooldown(f'mt:{base}', reason=f'http {cr.status_code}')
            raise RuntimeError(f'create account rate limited {base} status={cr.status_code}')
        if cr.status_code >= 400:
            last_err = RuntimeError(f'create account failed {base} status={cr.status_code}')
            continue
        tr = req.post(f'{base}/token', json={'address': email, 'password': password}, timeout=12)
        if _is_rate_limited_status(tr.status_code, tr.text):
            _provider_cooldown(f'mt:{base}', reason=f'token http {tr.status_code}')
            raise RuntimeError(f'token rate limited {base} status={tr.status_code}')
        if tr.status_code >= 400:
            last_err = RuntimeError(f'token failed {base} status={tr.status_code}')
            continue
        try:
            ...
            tok = ''
        if not tok:
            last_err = RuntimeError('no token')
            continue
        return f'mt|{base}|{tok}', email
    raise last_err or RuntimeError(f'no usable domain from {base}')
...
def _lol_create():
    ...
    if _is_rate_limited_status(r.status_code, r.text):
        _provider_cooldown('lol', reason=f'http {r.status_code}')
        raise RuntimeError(f'lol create rate limited http {r.status_code}')
    if r.status_code >= 400:
        raise RuntimeError(f'lol create http {r.status_code}')
    data = r.json()
    addr, tok = data.get('address', ''), data.get('token', '')
    if not addr or not tok:
        raise RuntimeError('lol create failed')
    if _domain_is_rejected(addr):
        raise RuntimeError(f'lol domain rejected {_email_domain(addr)}')
    return f'lol|{tok}', addr


def _tempmailio_create():
    """temp-mail.io 建箱;返回 (handle, email)。"""
    _throttle_email_create()
    r = req.post('https://api.internal.temp-mail.io/api/v3/email/new', timeout=12)
    if _is_rate_limited_status(r.status_code, r.text):
        _provider_cooldown('io', reason=f'http {r.status_code}')
        raise RuntimeError(f'tempmailio create rate limited http {r.status_code}')
    if r.status_code >= 400:
        raise RuntimeError(f'tempmailio create http {r.status_code}')
    data = r.json() if r.content else {}
    addr = data.get('email') or data.get('address') or ''
    tok = data.get('token') or ''
    if not addr or not tok:
        raise RuntimeError('tempmailio create failed')
    if _domain_is_rejected(addr):
        raise RuntimeError(f'tempmailio domain rejected {_email_domain(addr)}')
    return f'io|{addr}|{tok}', addr


def create_email():
    """custom 用自建域名(本地 webhook);tempmail 按可用 provider 顺序 fallback。"""
    ...
    password = rand_str()
    order = [x.strip() for x in TEMPMAIL_PROVIDER_ORDER.split(',') if x.strip()] or ['lol']
    makers = []
    seen = set()

    def add_lol():
        if 'lol' not in seen:
            makers.append(('lol', _lol_create))
            seen.add('lol')

    def add_io():
        if 'io' not in seen:
            makers.append(('io', _tempmailio_create))
            seen.add('io')
    ...
    def add_mailtm():
        for base in TEMPMAIL_BASES:
            mk = f'mt:{base}'
            if mk not in seen:
                makers.append((mk, (lambda b=base: _mailtm_create(b, password))))
                seen.add(mk)

    for name in order:
        key = name.lower()
        if key in ('lol', 'tempmail.lol', 'tempmail_lol'):
            add_lol()
        elif key in ('io', 'tempmailio', 'temp-mail.io', 'tempmail.io'):
            add_io()
        elif key in ('mailtm', 'mail.tm', 'mail'):
            add_mailtm()

    # 仅在 order 未覆盖时补齐, 不再无条件塞 duckmail。
    if not makers:
        add_io()
        add_mailtm()
        add_lol()
    ...
    errors = []
    for _ in range(EMAIL_CREATE_MAX_ATTEMPTS):
        progressed = False
        for name, make in makers:
            ...
            try:
                handle, email = make()
                if _domain_is_rejected(email):
                    raise RuntimeError(f'domain blacklisted {_email_domain(email)}')
                return handle, email, password
            except Exception as exc:
                msg = str(exc)
                errors.append(f'{name}: {exc}')
                if 'rate limited' in msg.lower() or '429' in msg:
                    _provider_cooldown(name, reason=msg[:120])
                continue
        if not progressed:
            with _PROVIDER_COOLDOWN_LOCK:
                waits = [until - time.time() for until in _PROVIDER_COOLDOWN_UNTIL.values() if until > time.time()]
            if waits:
                time.sleep(min(max(min(waits), 0.2), 5.0))
                continue
            break
    raise RuntimeError('所有临时邮箱 provider 均不可用 | ' + ' ; '.join(errors[-6:]))
...
def _tempmail_fetch(handle):
    ...
    if kind == 'io':
        ...
        items = data if isinstance(data, list) else data.get('messages') or data.get('emails') or []
        if not items:
            return None
        chunks = []
        for item in items:
            if not isinstance(item, dict):
                continue
            chunks.append(
                f"{item.get('subject','')}\n{item.get('body_text') or item.get('body') or item.get('text') or ''}\n{item.get('body_html') or item.get('html') or ''}"
            )
...
async def _send_q_request_batch(browser, physical_sem, p_send_sem, requests, metrics=None):
    ...
    try:
        ...
        for item in requests:
            current = dict(item)
            sent = False
            for attempt in range(CREATE_CODE_MAX_RETRIES):
                if _domain_is_rejected(current.get("email", "")):
                    try:
                        ...
                        debug_log(f"[P] recreate email after blacklist failed: {sanitize_terminal_error(exc)}")
                        break
                try:
                    sent = await grpc_create_code(page, current["email"])
                except asyncio.CancelledError:
                    raise
                except Exception:
                    sent = False
                if sent:
                    break
                # 域名被拒时换邮箱再试; 其他失败不盲重试浪费配额
                if _domain_is_rejected(current.get("email", "")):
                    continue
                break
            results.append({**current, "sent": sent})
        if metrics is not None:
        ...
        try:
            ...
            for _ in range(batch_count):
                ...
                try:
                    ...
                    metrics.p_email_create_count += 1
                    metrics.p_email_create_seconds += time.time() - email_started
                    debug_log(f'[P] {wid} create email err: {sanitize_terminal_error(e)} detail={str(e)[:220]}')
                    metrics.q_discarded += 1
                    q_pending_sem.release()
                    pending_owned -= 1
